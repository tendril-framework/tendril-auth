...
``tendril-auth`` can be installed normally from the Python Package Index.
...
`github releases <https://github.com/tendril-framework/tendril-auth/releases>`_.
...
``tendril-auth``. ``sudo`` may be necessary if you are not installing
into a virtual environment.

.. code-block:: console

    $ git clone https://github.com/tendril-framework/tendril-auth.git
    $ cd tendril-auth
    $ pip install -e .
